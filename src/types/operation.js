...
function Operation(operation, operationOptions = {}) {
  return function (options = {}) {
    ...
    try {
      ...
      permissions.current.any = options.acl[operation + "Any"](
        options.resource
      );
      permissions.current.own = options.acl[operation + "Own"](
        options.resource
      );
    } catch (e) {
...
function findPermission({ data, options, permissions }) {
  return data instanceof Array
    ? Promise.all(data.map(checkPermission))
...
function performAction({ data, options, permissions, operation }) {
  ...
  async function performActionSingle(entry) {
    ...
    if (operation === "create")
      return Action(permission && permission.filter(entry.data), meta);
    if (operation === "read") return entry.data;
    if (operation === "update")
      return Action(
        entry.data,
        permission && permission.filter(options.data),
        meta
      );
...
function OptionValidator(operation, options) {
  ...
  if (
    ...
    return new Error(
      "Insufficient options - required: " + JSON.stringify(props)
    );
  return;
}
